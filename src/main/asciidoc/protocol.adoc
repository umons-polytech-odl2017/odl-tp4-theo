...
Un flux classique utilisé depuis le début des séances de TP est System.out ; ce dernier permet d'envoyer des informations vers la sortie standard (la console).
On distingue également System.in (flux d'entrée standard : le clavier) et System.err (flux d'erreur standard : la console).
La gestion de fichier se fait à l'aide des classes du package java.io.
Les classes suivantes sont couramment utilisées :
